...
$woocommerce_post_types = [
    'shop_order' => __('Orders', 't9admin-pro'),
    'shop_coupon' => __('Coupons', 't9admin-pro'),
    'product' => __('Products', 't9admin-pro'),
    'shop_customer' => __('Customers', 't9admin-pro')
];
...
$menu_items = get_option('t9admin_pro_menu_items', []); // Load saved menu order

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $menu_items = isset($_POST['t9admin_pro_menu_items']) ? $this->sanitize_menu_items($_POST['t9admin_pro_menu_items']) : [];
    update_option('t9admin_pro_menu_items', $menu_items);
    ...
    add_action('admin_notices', function () {
        echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Menu saved successfully.', 't9admin-pro') . '</p></div>';
    });
}
?>

<div class="wrap">
    <h2><?php esc_html_e('Menu Settings', 't9admin-pro'); ?></h2>
    <form method="post" action="options.php">
        <div class="menu-settings-container">
            <!-- Left: Available Post Types -->
            <div style="flex: 1;">
                <h3><?php esc_html_e('Available Post Types', 't9admin-pro'); ?></h3>
                <ul id="t9admin-pro-available-items">
                    <li><strong><?php esc_html_e('WooCommerce', 't9admin-pro'); ?></strong></li>
                    <?php foreach ($woocommerce_post_types as $post_type => $label): ?>
                        ...
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <li><strong><?php esc_html_e('Custom Post Types', 't9admin-pro'); ?></strong></li>
            ...
            <!-- Right: Current Menu -->
            <div style="flex: 2;">
                <h3><?php esc_html_e('Current Menu', 't9admin-pro'); ?></h3>
                <?php
                settings_fields('t9admin_pro_menu_settings');
                do_settings_sections('t9admin_pro_menu_settings');
                ?>
                <div class="menu-right-container">
                    <ul id="t9admin-pro-menu-list" class="sortable-menu-list">
                        <?php 
                        ...
                        ?>
                            <li class="menu-item <?php echo esc_attr($indent_class); ?>" data-post-type="<?php echo esc_attr($menu_item['post_type']); ?>" data-parent-index="<?php echo $is_child ? esc_attr($current_parent_index) : ''; ?>">
                                ...
                                <?php elseif ($menu_item['post_type'] === 'label'): ?>
                                    <input type="text" name="t9admin_pro_menu_items[<?php echo esc_attr($key); ?>][label]" 
                                        value="<?php echo esc_attr($menu_item['label']); ?>" 
                                        placeholder="<?php esc_attr_e('Enter Label', 't9admin-pro'); ?>" 
                                        class="menu-label-input">
                                <?php elseif ($menu_item['post_type'] === 'mini-nav'): ?>
                                    <select name="t9admin_pro_menu_items[<?php echo esc_attr($key); ?>][icon]" class="menu-icon-select">
                                        <option value=""><?php esc_html_e('Select Icon', 't9admin-pro'); ?></option>
                                    ...
                                    <input type="text" name="t9admin_pro_menu_items[<?php echo esc_attr($key); ?>][label]" 
                                        value="<?php echo esc_attr($menu_item['label']); ?>" 
                                        placeholder="<?php esc_attr_e('Enter Mini Nav Label', 't9admin-pro'); ?>" 
                                        class="menu-label-input">
                                <?php else: ?>
                                    <select name="t9admin_pro_menu_items[<?php echo esc_attr($key); ?>][icon]" class="menu-icon-select">
                                        <option value=""><?php esc_html_e('Select Icon', 't9admin-pro'); ?></option>
                                ...
                                <input type="hidden" name="t9admin_pro_menu_items[<?php echo esc_attr($key); ?>][post_type]" value="<?php echo esc_attr($menu_item['post_type']); ?>">
                                <?php if ($menu_item['post_type'] !== 'label' && $menu_item['post_type'] !== 'mini-nav'): ?>
                                    <input type="hidden" name="t9admin_pro_menu_items[<?php echo esc_attr($key); ?>][label]" value="<?php echo esc_attr($menu_item['label']); ?>">
                                <?php endif; ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
                <button type="button" id="add-label" class="button"><?php esc_html_e('Add Label', 't9admin-pro'); ?></button>
                <button type="button" id="add-hr" class="button"><?php esc_html_e('Add Horizontal Line', 't9admin-pro'); ?></button>
                <button type="button" id="add-mini-nav" class="button"><?php esc_html_e('Add Mini Nav', 't9admin-pro'); ?></button>
            </div>
        </div>
        <?php submit_button(); ?>
    </form>
...
jQuery(document).ready(function ($) {
    let menuIndex = $('#t9admin-pro-menu-list .menu-item').length;

    // Thêm item từ Available Post Types
    $('#t9admin-pro-available-items').on('click', '.available-item', function () {
        ...
        const newItem = `
            <li class="menu-item child-menu" data-post-type="${postType}" data-parent-index="${getLastMiniNavIndex()}">
                <span class="dashicons dashicons-menu"></span>
                <select name="t9admin_pro_menu_items[${menuIndex}][icon]" class="menu-icon-select">
                    <option value=""><?php esc_html_e('Select Icon', 't9admin-pro'); ?></option>
                    <?php foreach ($this->get_bootstrap_icons() as $icon): ?>
                ...
                <input type="hidden" name="t9admin_pro_menu_items[${menuIndex}][post_type]" value="${postType}">
                <input type="hidden" name="t9admin_pro_menu_items[${menuIndex}][label]" value="${label}">
            </li>
        `;

        const lastMiniNav = $('#t9admin-pro-menu-list .menu-item[data-post-type="mini-nav"]').last();
        if (lastMiniNav.length) {
            lastMiniNav.after(newItem);
        } else {
            $('#t9admin-pro-menu-list').append(newItem);
        }
        menuIndex++;
        $(this).remove();
    });
    ...
    $('#t9admin-pro-menu-list').sortable({
        placeholder: "ui-state-highlight",
        handle: ".dashicons-menu",
        stop: function () {
            let menuIndex = 0;
            let currentParentIndex = null;
            $('#t9admin-pro-menu-list .menu-item').each(function () {
    ...
    $('#add-label').on('click', function () {
        const newItem = `
            <li class="menu-item child-menu" data-post-type="label" data-parent-index="${getLastMiniNavIndex()}">
                <span class="dashicons dashicons-menu"></span>
                <input type="text" name="t9admin_pro_menu_items[${menuIndex}][label]" 
                    placeholder="Enter label text" class="menu-label-input" 
                    value="">
                <button type="button" class="button remove-menu-item">
                    <span class="dashicons dashicons-minus"></span>
                </button>
                <input type="hidden" name="t9admin_pro_menu_items[${menuIndex}][post_type]" value="label">
            </li>
        `;
        const lastMiniNav = $('#t9admin-pro-menu-list .menu-item[data-post-type="mini-nav"]').last();
        if (lastMiniNav.length) {
    ...
    $('#add-hr').on('click', function () {
        const newItem = `
            <li class="menu-item child-menu" data-post-type="hr" data-parent-index="${getLastMiniNavIndex()}">
                ...
                <input type="hidden" name="t9admin_pro_menu_items[${menuIndex}][post_type]" value="hr">
                <input type="hidden" name="t9admin_pro_menu_items[${menuIndex}][label]" value="---------------">
            </li>
        `;
        const lastMiniNav = $('#t9admin-pro-menu-list .menu-item[data-post-type="mini-nav"]').last();
        if (lastMiniNav.length) {
    ...
    $('#add-mini-nav').on('click', function () {
        const newItem = `
            <li class="menu-item" data-post-type="mini-nav">
                ...
                <input type="hidden" name="t9admin_pro_menu_items[${menuIndex}][post_type]" value="mini-nav">
            </li>
        `;
        $('#t9admin-pro-menu-list').append(newItem);
        menuIndex++;
    ...
    // Xóa menu item
    $('#t9admin-pro-menu-list').on('click', '.remove-menu-item', function () {
        ...
        if (postType && label && postType !== 'hr' && postType !== 'label' && postType !== 'mini-nav') {
            ...
            $('#t9admin-pro-available-items').append(newAvailableItem);
        }
...
#t9admin-pro-menu-list .menu-item {
    display: flex;
    align-items: center;
    padding: 10px;
    margin-bottom: 5px;
...
#t9admin-pro-menu-list .menu-item.child-menu {
    margin-left: 20px;
    background: #f9f9f9;
}

#t9admin-pro-menu-list .menu-item .dashicons-menu {
    cursor: grab;
}

#t9admin-pro-menu-list .menu-item .menu-icon-select {
    margin: 0 8px 0 16px;
}

#t9admin-pro-menu-list .menu-item .remove-menu-item {
    border: none;
    background: none;
    color: #ff0000;
    border-radius: 0;
...
#t9admin-pro-available-items {
    list-style: none;
    padding: 0;
}

#t9admin-pro-available-items .available-item {
    padding: 10px;
    margin-bottom: 5px;
    background: #f7f7f7;
    border: 1px solid #ddd;
...
#t9admin-pro-available-items .available-item .dashicons-plus {
    margin-right: 10px;
}
