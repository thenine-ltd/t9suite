document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".dropzone").forEach(function(dropzoneElement) {
        let fieldName = dropzoneElement.dataset.field;
        new Dropzone("#dropzone-" + fieldName, {
            url: "/wp-admin/admin-ajax.php?action=upload_image", // Thay bằng endpoint của bạn
            paramName: "file",
            maxFilesize: 2, // MB
            acceptedFiles: "image/*",
            init: function() {
                this.on("success", function(file, response) {
                    ...
                    // TODO: Xử lý lưu URL ảnh vào hidden input nếu cần
                });
            }
        });
    });
...
function printQRCode() {
    var qrCodeUrl = "<?php echo esc_url($qr_code_url); ?>"; // Nhúng URL QR từ PHP vào JS
    ...
    qrWindow.document.write('<img src="' + qrCodeUrl + '" style="max-width:100%;">'); // Dùng chuỗi JS
    qrWindow.document.write('<br><button onclick="window.print()">Print</button>');
